...
domReady(() => {
	...
	// Init all accordions.
	const accordionButtons = document.querySelectorAll('.hizzle-accordion__heading');
	...
	accordionButtons.forEach((button) => {
		...
		try {
			...
			button.addEventListener('click', (event) => {
				...
				closest( button, '.hizzle-accordion__heading', toggleAccordion );
			});
	...
	const resizeAccordionPanels = () => {
		const accordionPanels = document.querySelectorAll('.hizzle-accordion__panel');

		accordionPanels.forEach((panel) => {
			panel.style.setProperty( '--hizzle-accordion-panel-max-height', panel.scrollHeight + 'px' );
		}
	)};
